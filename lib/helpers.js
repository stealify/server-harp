...
exports.log = function(promise) {
  return promise
    ...
    .catch(function(error) {
      ...
      debug('log:error')(error.stack || error.message || error);
...
exports.setup = function(projectPath, env){
  ...
  var configMode = 'json';
  var configPath,contents,publicPath;
  try{

    configPath  = path.join(projectPath, 'steal.json');
    contents    = fs.readFileSync(configPath).toString();
    publicPath  = path.join(projectPath, 'public');

  }catch(e){
    try{
      configPath  = path.join(projectPath, 'steal.js');
      contents    = fs.readFileSync(configPath).toString();
      publicPath  = projectPath;
      configMode = 'js';
    }catch(e){
      try{
        configPath  = path.join(projectPath, '_steal.json');
        contents    = fs.readFileSync(configPath).toString();
        publicPath  = projectPath;
      }catch(e){
        try{
          configPath  = path.join(projectPath, '_steal.js');
          contents    = fs.readFileSync(configPath).toString();
          publicPath  = projectPath;
          configMode = 'js';
        }catch(e){
  ...
  // attempt to parse the file
    ...
    try{
      ...
      e.stack     = contents;
      e.lineno    = -1;
      throw new prerender.helpers.stealPreRenderError(e);
    }
...
exports.prime = function(primePath, options, callback){
  ...
  primePath = path.resolve(primePath);
