...
exports.setup = function(projectPath, env){
  ...
  try{
    ...
    configPath  = path.join(projectPath, 'dssrv.json');
    contents    = fs.readFileSync(configPath).toString();
    publicPath  = path.join(projectPath, 'public');

  }catch(e){
    try{
      configPath  = path.join(projectPath, 'dssrv.js');
      contents    = fs.readFileSync(configPath).toString();
      publicPath  = projectPath;
      configMode = 'js';
    }catch(e){
      try{
        configPath  = path.join(projectPath, '_dssrv.json');
        contents    = fs.readFileSync(configPath).toString();
        publicPath  = projectPath;
      }catch(e){
        try{
          configPath  = path.join(projectPath, '_dssrv.js');
          contents    = fs.readFileSync(configPath).toString();
          publicPath  = projectPath;
          configMode = 'js';
        }catch(e){
