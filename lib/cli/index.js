...
exports.run = function(app){
  ...
  let server = servers.filter(server=>server.address().port === port)
  if (server.length === 1) {
    return server[0]
  }

	function startServer(app){
		app = app || makeServer(options)
		let server =servers.filter(server=>server.address().port === port)
		 if (server.length === 1) {
			return server[0]
		 }
		...
		if(options.key && options.cert) {
			...
			let httpsServerIndex = servers.push(spdy.createServer({
			...
			servers[httpsServerIndex].listen(httpsPort);
			let httpServerIndex = servers.push(require("http").createServer(function(req, res){
				var host = req.headers.host;
				res.writeHead(301, { "Location": "https://" + host + req.url });
				res.end();
			}))

			servers[httpServerIndex].listen(httpPort)
			servers[httpServerIndex].on('error', function(e) {
	...
	servers[httpServerIndex].on('listening', function() {
		...
		console.log('done-serve starting on ' + url);
			...
			// http or https
		let tcpServerIndex = servers.push(net.createServer(function(conn){
		...
		servers[tcpServerIndex].listen(port);
		} else {
  ...
  return startServer(makeServer(options));
};
