...
//       middleware.hostProjectFinder(dirPath),
  ...
  this.pipeline = function(root){
    debug('pipeline')('Deprecated, please use MOUNT instead, this will be removed in a future version.');
        ...
        async.each(results, compileFile, function(err){
          if(err){
            ...
            async.each(results, copyFile, function(err){
              if (!err) {
                setup.config['steal_version'] = pkg.version;
                // setup.config['steal_version'] = require('../package.json').version
